...
class Config:
    # ---- 环境 ----
    xml_path = "/home/lulsong/WorkTask/chain-of-action/sim/assets/tmp.xml"
...
def load_planning_head(plan_ckpt, meta_json, device):
    ...
    max_len = int(meta.get("max_len", 9))
...
def load_skill_expert(ckpt_path, device):
    ...
    if "head" in ckpt:         # 你训练脚本的存法
        ...
        d_in   = fuse_w.shape[1]
        # 猜测参数（不完美，但能跑）
        bb_d_out = 1024; d_hidden=1024; action_dim=32; use_gru=True; skill_vocab=9; d_skill=64
...
def main():
    ...
    # img0 = env.render_rgb(camera_name='wrist1')
    img0 = env.render_rgb()
    st0  = env.get_state()
    ...
    skill_seqs = greedy_with_constraints(
        plan_head, H_txt0_plan, H_v0_plan, S0_plan, K, trans_mask, max_len=meta.get("max_len", 9)
    )
