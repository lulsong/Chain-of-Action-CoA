...
class EpisNPZ(Dataset):
    def __init__(self, npz_dir: str, max_out_len=9):
...
# ---- 修正后的 collate 函数：不再处理 state ----
def collate(batch, K, max_out_len):
...
class CondProject(nn.Module):
    ...
    def forward(self, H_txt, H_vs):
        mem = torch.cat([H_txt, H_vs], dim=1)
        Tm = mem.size(1)
        return self.ln(mem + self.pos[:, :Tm, :])

class SkillFormer(nn.Module):
    def __init__(self, K, d=1024, n_layers=4, n_heads=4, d_ff=4096, max_len=32, dropout=0.1):
        super().__init__()
        self.K = K; self.d = d
    ...
    def forward(self, H_txt, H_vs, Yin):
        B, L = Yin.size()
        mem = self.cond(H_txt, H_vs)
...
# ------------------- Constraint Decoding -------------------
@torch.no_grad()
def greedy_with_constraints(model, H_txt, H_vs, K, trans_mask, max_len=32, temperature=1.0):
    B = H_txt.size(0)
    ...
    for _ in range(max_len):
        lg = model(H_txt, H_vs, Yin)[:, -1, :] / max(1e-6, temperature)
        lg[:, PAD] = -1e9
        if prev is not None:
...
# ------------------- Main -------------------
def main(data_dir: str, out_dir: str, epochs=15, batch_size=128, lr=3e-4, d=1024, max_out_len=9, seed=42):
    set_seed(seed)
    ...
    coll = lambda b: collate(b, K, max_out_len)  # Collate 修正：去掉 state 参数
    ...
    model = SkillFormer(K, d=d, n_layers=4, n_heads=4, d_ff=4*d, max_len=max_out_len, dropout=0.1).to(device)
    ...
    for ep in range(1, epochs+1):
        ...
        for batch in tr:
            batch=to_dev(batch)
            logits=model(batch["H_txt"],batch["H_vs"],batch["Yin"])
        ...
        with torch.no_grad():
            for batch in va:
                ...
                preds=greedy_with_constraints(model,batch["H_txt"],batch["H_vs"],K,trans_mask,max_len=max_out_len)
        ...
        # ---- save ----
        torch.save({"model":model.state_dict(),"K":K,"cfg":{"d":d,"max_out_len":max_out_len}},
...
if __name__ == "__main__":
    ...
    ap.add_argument("--max_out_len",type=int,default=9)
