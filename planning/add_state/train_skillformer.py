...
class EpisNPZ(Dataset):
    ...
    def __getitem__(self, i):
        ...
        seq = seq[:self.max_out_len-1]  # 留一个位置给 EOS
        # 允许 state 为空数组
        st = z["state"]
...
def collate(batch, K, max_out_len, state_dim, state_mean, state_std):
    ...
    H_vgs  = [b["H_vg"] for b in batch]
    X_txt, L_txt = pad_nd(H_txts)
    X_vs,  L_vs  = pad_nd(H_vss)
    X_vg,  L_vg  = pad_nd(H_vgs)
    ...
    Yin = np.full((len(batch), L), K+2, dtype=np.int64)  # PAD = K+2
    ...
    return {
        ...
        "H_vg":  torch.from_numpy(X_vg),  "L_vg":  torch.from_numpy(L_vg),
...
class CondProject(nn.Module):
    """把 memory 映射到同一维度后拼接，并加位置编码"""
    def __init__(self, d):
        super().__init__()
        self.ln = nn.LayerNorm(d)
        self.pos = nn.Parameter(torch.randn(1, 2048, d) * 0.01)  # 上限够用
    def forward(self, H_txt, H_vs, H_vg, H_state):
        mem = torch.cat([H_txt, H_vs, H_vg, H_state], dim=1)  # [B, Tm, d]
        Tm = mem.size(1)
        return self.ln(mem + self.pos[:, :Tm, :])

class StateMLP(nn.Module):
    """d_state -> D，输出 [B,1,D] 的 state token"""
    ...
    def forward(self, s):  # s: [B, d_state]
        x = self.net(s)    # [B, D]
...
class SkillFormer(nn.Module):
    def __init__(self, K, d=256, n_layers=4, n_heads=4, d_ff=1024, max_len=64, dropout=0.1, d_state=0):
        super().__init__()
        self.K = K; self.d = d
        self.embed_out = nn.Embedding(K+3, d)   # K: BOS, K+1: EOS, K+2: PAD
    ...
    def forward(self, H_txt, H_vs, H_vg, Yin, S=None):
        ...
        if self.state_mlp is not None and S is not None:
            H_state = self.state_mlp(S)               # [B,1,d]
        else:
            H_state = torch.zeros((B,1,self.d), device=Yin.device, dtype=torch.float32)
        mem = self.cond(H_txt, H_vs, H_vg, H_state)   # [B,Tm,d]
        ...
        return self.fc(out)                           # [B,L,K+3]

# -------- Constraint decoding --------
@torch.no_grad()
def greedy_with_constraints(model, H_txt, H_vs, H_vg, S, K, trans_mask, max_len=32, temperature=1.0):
    B = H_txt.size(0)
    ...
    for _ in range(max_len):
        lg = model(H_txt, H_vs, H_vg, Yin, S)[:, -1, :] / max(1e-6, temperature)  # [B,K+3]
        lg[:, PAD] = -1e9
        if prev is not None:
...
def ngram_f1(pred: List[int], gold: List[int], n=2):
    from collections import Counter
    def grams(x):
        return Counter([tuple(x[i:i+n]) for i in range(len(x)-n+1)]) if len(x)>=n else Counter()
    P = grams(pred); G = grams(gold)
    inter = sum((P & G).values()); p = sum(P.values()); g = sum(G.values())
    if p==0 or g==0: return 0.0
    prec, rec = inter/p, inter/g
    return 2*prec*rec/(prec+rec+1e-9)

def main(data_dir: str, out_dir: str, epochs=15, batch_size=128, lr=3e-4, d=256, max_out_len=32, seed=42):
    set_seed(seed)
    ...
    trans_mask = (trans > 0)  # [K,K] bool
    trans_mask = torch.from_numpy(trans_mask).to(device)
    ...
    for ep in range(1, epochs+1):
        ...
        for batch in tr:
            batch = to_dev(batch)
            logits = model(batch["H_txt"], batch["H_vs"], batch["H_vg"], batch["Yin"], batch["state"])
        ...
        # val
        model.eval()
        em=0; lev=0; f1=0; M=0; vloss=0.0; VN=0
        with torch.no_grad():
            for batch in va:
                batch = to_dev(batch)
                logits = model(batch["H_txt"], batch["H_vs"], batch["H_vg"], batch["Yin"], batch["state"])
                ...
                # greedy + constraints
                preds = greedy_with_constraints(model, batch["H_txt"], batch["H_vs"], batch["H_vg"], batch["state"], K, trans_mask, max_len=max_out_len)
        ...
        torch.save({
            ...
            "K": K,
            "cfg": {"d": d, "max_out_len": max_out_len, "state_dim": state_dim},
...
if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser("Train SkillFormer (planning head, +proprio state)")
    ap.add_argument("--data_dir", required=True, help="prep_plan_data.py 输出目录")
    ap.add_argument("--out_dir", default="./plan_ckpt")
